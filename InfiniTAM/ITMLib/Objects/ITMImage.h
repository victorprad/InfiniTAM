	...
	namespace Objects
	{
		...
		*/
		...
		public:
			...
			on CPU only or on both CPU and GPU.
			*/
			ITMImage(Vector2i noDims, bool allocate_CPU, bool allocate_CUDA, bool metalCompatible = false)
				: ITMMemoryBlock(noDims.x * noDims.y, allocate_CPU, allocate_CUDA, metalCompatible)
			{
				this->noDims = noDims;
			}

			ITMImage(bool allocate_CPU, bool allocate_CUDA, bool metalCompatible = false) 
