...
template<class TVoxel>
ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHHash>::State::State(const Vector2i & imgSize)
 : ITMVisualisationState(imgSize, false)
{
...
template<class TVoxel>
ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHHash>::State::~State(void)
{
	delete[] entriesVisibleType;
	delete[] visibleEntryIDs;
}
...
template<class TVoxel>
void ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHHash>::FindVisibleBlocks(const ITMScene<TVoxel,ITMVoxelBlockHHash> *scene, const ITMPose *pose, const ITMIntrinsics *intrinsics, ITMVisualisationState *_state)
{
	State *state = (State*)_state;
	const ITMHashEntry *hashTable = scene->index.GetEntries();
	int noTotalEntries = scene->index.noVoxelBlocks;
	...
	for (int targetIdx = 0; targetIdx < noTotalEntries; targetIdx++)
	{
		int level = scene->index.getIndexData()->GetLevelForEntry(targetIdx);
...
template<class TVoxel>
void ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHHash>::CreateExpectedDepths(const ITMScene<TVoxel,ITMVoxelBlockHHash> *scene, const ITMPose *pose, const ITMIntrinsics *intrinsics, ITMImage<Vector2f> *minmaximg, const ITMVisualisationState *state)
{
	Vector2i imgSize = minmaximg->noDims;
	Vector2f *minmaxData = minmaximg->GetData(false);
	...
	for (int blockNo = 0; blockNo < noLiveEntries; ++blockNo) {
		...
		int level = scene->index.getIndexData()->GetLevelForEntry(blockId);
		float voxelSize = smallestVoxelSize * (1 << level);
...
template<class TVoxel>
void ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHHash>::RenderImage(const ITMScene<TVoxel,ITMVoxelBlockHHash> *scene, const ITMPose *pose, const ITMIntrinsics *intrinsics, const ITMVisualisationState *state, ITMUChar4Image *outputImage, bool useColour)
{
	RenderImage_common(scene, pose, intrinsics, state, outputImage, useColour);
}

template<class TVoxel>
void ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHHash>::CreatePointCloud(const ITMScene<TVoxel,ITMVoxelBlockHHash> *scene, const ITMView *view, ITMTrackingState *trackingState, bool skipPoints)
{
	CreatePointCloud_common(scene, view, trackingState, skipPoints);
}

template<class TVoxel>
void ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHHash>::CreateICPMaps(const ITMScene<TVoxel,ITMVoxelBlockHHash> *scene, const ITMView *view, ITMTrackingState *trackingState)
{
	CreateICPMaps_common(scene, view, trackingState);
}
