...
template<class TVoxel>
void ITMSceneReconstructionEngine_CPU<TVoxel,ITMVoxelBlockHHash>::IntegrateIntoScene(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene, const ITMView *view, const ITMPose *pose_d)
{
	Vector2i rgbImgSize = view->rgb->noDims;
	Vector2i depthImgSize = view->depth->noDims;
	...
	for (int entryNo = 0; entryNo < scene->index.noLiveEntries; entryNo++)
	{
		Vector3i globalPos;
		int entryId = liveEntryIDs[entryNo];
		const ITMHashEntry &currentHashEntry = hashTable[entryId];

		if (currentHashEntry.ptr < 0) continue;

		float localVoxelSize = smallestVoxelSize * (1 << scene->index.getIndexData()->GetLevelForEntry(entryId));
...
template<class TVoxel>
void ITMSceneReconstructionEngine_CPU<TVoxel,ITMVoxelBlockHHash>::AllocateSceneFromDepth(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene, const ITMView *view, const ITMPose *pose_d)
{
	Vector2i depthImgSize = view->depth->noDims;
	float smallestVoxelSize = scene->sceneParams->voxelSize;
