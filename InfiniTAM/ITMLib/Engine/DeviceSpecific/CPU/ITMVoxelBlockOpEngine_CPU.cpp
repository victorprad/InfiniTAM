...
template<class TVoxel>
void ITMVoxelBlockOpEngine_CPU<TVoxel,ITMVoxelBlockHHash>::ComputeComplexities(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene)
{
...
template<class TVoxel>
void ITMVoxelBlockOpEngine_CPU<TVoxel,ITMVoxelBlockHHash>::SplitVoxelBlocks(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene)
{
...
template<class TVoxel>
void ITMVoxelBlockOpEngine_CPU<TVoxel,ITMVoxelBlockHHash>::MergeVoxelBlocks(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene)
{
	TVoxel *voxelBlocks = scene->localVBA.GetVoxelBlocks();
	ITMHashEntry *allHashEntries = scene->index.GetEntries();
	int *voxelAllocationList = scene->localVBA.GetAllocationList();
	...
	for (int htIdx = ITMVoxelBlockHHash::IndexData::noTotalEntriesPerLevel; htIdx < ITMVoxelBlockHHash::IndexData::noTotalEntries; htIdx++)
	{
		int blockId = allHashEntries[htIdx].ptr;
		// only merge, if the block was previously split
		if (blockId != -2) continue;

		int parentLevel = scene->index.getIndexData()->GetLevelForEntry(htIdx);
		int childLevel = parentLevel-1;
		ITMHashEntry *childHashTable = &(allHashEntries[ITMVoxelBlockHHash::IndexData::noTotalEntriesPerLevel * childLevel]);
...
template<class TVoxel>
void ITMVoxelBlockOpEngine_CPU<TVoxel,ITMVoxelBlockHHash>::SplitAndMerge(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene)
{
