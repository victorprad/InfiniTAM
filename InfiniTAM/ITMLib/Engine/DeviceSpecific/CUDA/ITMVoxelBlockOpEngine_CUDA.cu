...
ITMVoxelBlockOpEngine_CUDA<TVoxel,ITMVoxelBlockHHash>::ITMVoxelBlockOpEngine_CUDA(void)
{
	...
	ITMSafeCall(cudaMalloc((void**)&noAllocatedExcessEntries_device, sizeof(int)*ITMVoxelBlockHHash::IndexData::noLevels));
}
...
template<class TVoxel>
void ITMVoxelBlockOpEngine_CUDA<TVoxel,ITMVoxelBlockHHash>::ComputeComplexities(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene)
{
	{
		dim3 blockSize(256);
	...
	const int *liveList = scene->index.GetLiveEntryIDs();
	int liveListSize = scene->index.noLiveEntries;
...
__global__ void createSplits_device(const int *liveList, int liveListSize, float *complexities, ITMHashEntry *allHashEntries, int *excessAllocationList, int *lastFreeExcessListIds, int *voxelAllocationList, int *lastFreeVoxelBlockId, int *blocklist, int *lastEntryBlockList)
{
	int listIdx = blockIdx.x * blockDim.x + threadIdx.x;
	if (listIdx >= liveListSize) return;

	int htIdx = liveList[listIdx];
	int parentLevel = ITMVoxelBlockHHash::IndexData::GetLevelForEntry(htIdx);
	...
	ITMHashEntry *childHashTable = &(allHashEntries[ITMVoxelBlockHHash::IndexData::noTotalEntriesPerLevel * childLevel]);
...
template<class TVoxel>
void ITMVoxelBlockOpEngine_CUDA<TVoxel,ITMVoxelBlockHHash>::SplitVoxelBlocks(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene)
{
	...
	if (liveListSize > 0) {
		...
		ITMSafeCall(cudaMemcpy(noAllocatedExcessEntries_device, lastFreeExcessListIds, ITMVoxelBlockHHash::IndexData::noLevels * sizeof(int), cudaMemcpyHostToDevice));
		...
		ITMSafeCall(cudaMemcpy(lastFreeExcessListIds, noAllocatedExcessEntries_device, ITMVoxelBlockHHash::IndexData::noLevels * sizeof(int), cudaMemcpyDeviceToHost));
	}
...
template<class TVoxel>
void ITMVoxelBlockOpEngine_CUDA<TVoxel,ITMVoxelBlockHHash>::MergeVoxelBlocks(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene)
{
	ITMHashEntry *allHashEntries = scene->index.GetEntries();
	TVoxel *voxelBlocks = scene->localVBA.GetVoxelBlocks();
	int *voxelAllocationList = scene->localVBA.GetAllocationList();
	...
	dim3 blockSize(256);
	dim3 gridSize(ceil((float)(ITMVoxelBlockHHash::IndexData::noTotalEntries - ITMVoxelBlockHHash::IndexData::noTotalEntriesPerLevel) / (float)blockSize.x));


	ITMSafeCall(cudaMemcpy(noAllocatedVoxelEntries_device, &lastFreeVoxelBlockId, sizeof(int), cudaMemcpyHostToDevice));
	ITMSafeCall(cudaMemcpy(noAllocatedExcessEntries_device, lastFreeExcessListIds, ITMVoxelBlockHHash::IndexData::noLevels * sizeof(int), cudaMemcpyHostToDevice));
	createMerges_device << <gridSize, blockSize >> >(ITMVoxelBlockHHash::IndexData::noTotalEntriesPerLevel, ITMVoxelBlockHHash::IndexData::noTotalEntries, complexities, allHashEntries, excessAllocationList, noAllocatedExcessEntries_device, voxelAllocationList, noAllocatedVoxelEntries_device, blocklist, blocklist_size_device);
	ITMSafeCall(cudaMemcpy(&lastFreeVoxelBlockId, noAllocatedVoxelEntries_device, sizeof(int), cudaMemcpyDeviceToHost));
	ITMSafeCall(cudaMemcpy(lastFreeExcessListIds, noAllocatedExcessEntries_device, ITMVoxelBlockHHash::IndexData::noLevels * sizeof(int), cudaMemcpyDeviceToHost));
...
template<class TVoxel>
void ITMVoxelBlockOpEngine_CUDA<TVoxel,ITMVoxelBlockHHash>::SplitAndMerge(ITMScene<TVoxel,ITMVoxelBlockHHash> *scene)
{
	ComputeComplexities(scene);
	SplitVoxelBlocks(scene);
	MergeVoxelBlocks(scene);
}
