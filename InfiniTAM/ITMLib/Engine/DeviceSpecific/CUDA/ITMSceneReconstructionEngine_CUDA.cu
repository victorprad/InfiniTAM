...
template<class TVoxel>
void ITMSceneReconstructionEngine_CUDA<TVoxel, ITMVoxelBlockHHash>::AllocateSceneFromDepth(ITMScene<TVoxel, ITMVoxelBlockHHash> *scene, const ITMView *view, const ITMPose *pose_d)
{
	Vector2i depthImgSize = view->depth->noDims;
	...
	for (int level = 0; level < noLevels; ++level) {
		...
		buildVisibleList_device << <gridSizeAL, cudaBlockSizeAL >> >(hashTable + levelOffset, cacheStates + levelOffset, scene->useSwapping, noTotalEntriesPerLevel, liveEntryIDs,
			noLiveEntries_device, entriesVisibleType + levelOffset, M_d, projParams_d, depthImgSize, voxelSize, levelOffset);
	}
...
template<class TVoxel>
void ITMSceneReconstructionEngine_CUDA<TVoxel, ITMVoxelBlockHHash>::IntegrateIntoScene(ITMScene<TVoxel, ITMVoxelBlockHHash> *scene, const ITMView *view, const ITMPose *pose_d)
{
	Vector2i rgbImgSize = view->rgb->noDims;
	Vector2i depthImgSize = view->depth->noDims;
	float smallestVoxelSize = scene->sceneParams->voxelSize;
	...
	Vector4f projParams_d, Vector4f projParams_rgb, const ITMVoxelBlockHHash::IndexData *indexData, float smallestVoxelSize, float mu, int maxW)
{
	...
	float localVoxelSize = smallestVoxelSize * (1 << indexData->GetLevelForEntry(entryId));
	...
	int *noAllocatedVoxelEntries, int *noAllocatedExcessEntries, uchar *entriesAllocType, uchar *entriesVisibleType, Vector3s *blockCoords)
{
	...
	case 2: //needs allocation in the excess list
		...
		if (vbaIdx >= 0 && exlIdx >= 0) //there is room in the voxel block array and excess list
		{
			...
			hashTable[level * ITMVoxelBlockHHash::IndexData::noTotalEntriesPerLevel + SDF_BUCKET_NUM * SDF_ENTRY_NUM_PER_BUCKET + exlOffset] = hashEntry; //add child to the excess list

			entriesVisibleType[level * ITMVoxelBlockHHash::IndexData::noTotalEntriesPerLevel + SDF_BUCKET_NUM * SDF_ENTRY_NUM_PER_BUCKET + exlOffset] = 1; //make child visible
		}
