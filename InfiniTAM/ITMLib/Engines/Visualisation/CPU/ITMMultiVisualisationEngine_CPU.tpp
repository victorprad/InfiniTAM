...
template<class TVoxel, class TIndex>
void ITMMultiVisualisationEngine_CPU<TVoxel, TIndex>::PrepareRenderState(const ITMMultiSceneManager_instance<TVoxel, TIndex> & sceneManager, ITMRenderState *_state)
{
	ITMRenderStateMultiScene<TVoxel, TIndex> *state = (ITMRenderStateMultiScene<TVoxel, TIndex>*)_state;

	state->PrepareLocalScenes(sceneManager);
}
...
#pragma omp parallel for
		...
		for (int locId = 0; locId < imgSize.x * imgSize.y; locId++) {
			int y = locId / imgSize.x;
			int x = locId - y*imgSize.x;
			processPixelNormals_ImageNormals<true>(outRendering, pointsRay, imgSize, x, y, voxelSize, lightSource);
		}
		break;
		...
		for (int locId = 0; locId < imgSize.x * imgSize.y; locId++) {
		Vector4f ptRay = pointsRay[locId];
		processPixelConfidence<ITMMultiVoxel<TVoxel>, ITMMultiIndex<TIndex> >(outRendering[locId], ptRay, ptRay.w > 0,
		&(renderState->voxelData_host), &(renderState->indexData_host), lightSource);
		}
		break;*/
...
#pragma omp parallel for
		...
		for (int locId = 0; locId < imgSize.x * imgSize.y; locId++) {
			int y = locId / imgSize.x;
			int x = locId - y*imgSize.x;
			processPixelGrey_ImageNormals<true>(outRendering, pointsRay, imgSize, x, y, voxelSize, lightSource);
		}
		break;
	}
}
