...
namespace ITMLib
{
	...
	template<class TVoxel, class TIndex>
	const ITMPoseConstraint & ITMMultiSceneManager_instance<TVoxel, TIndex>::getRelation_const(int fromScene, int toScene) const
	{
	...
	ORUtils::SE3Pose ITMMultiSceneManager_instance<TVoxel, TIndex>::findTransformation(int fromSceneID, int toSceneID) const
	{
		...
		for (size_t i = 0; i + 1 < pathThroughScenes.size(); ++i) {
			...
			ConstraintList::const_iterator it = allData[currentScene]->relations.find(nextScene);
			// it should never be invalid, as the path was just
			// constructed in that way
			ret.SetM(it->second.GetAccumulatedObservations().GetM() * ret.GetM());
			ret.Coerce();
		}
